...
namespace cs251
{
  ...
  dominos_t::dominos_t()
  {
    ...
    // The pendulum that knocks the dominos off
    {
      ...
      b2->CreateFixture(&shape, 20.0f);
      }
  
      b2Body* b4;
      {
      b2PolygonShape shape;
      shape.SetAsBox(1.25f, 0.25f);
    ...
    //1st revolving verical shelf
    {
      b2PolygonShape shape;
      shape.SetAsBox(0.2f, 2.0f);
  
      b2BodyDef bd;
      bd.position.Set(-24.0f, 29.0f);
      bd.type = b2_dynamicBody;
      b2Body* body = m_world->CreateBody(&bd);
    ...
    // 3rd revolving verical shelf
    {
      b2PolygonShape shape;
      shape.SetAsBox(0.2f, 2.0f);
  
      b2BodyDef bd;
      bd.position.Set(-24.0f, 23.0f);
      bd.type = b2_dynamicBody;
      b2Body* body = m_world->CreateBody(&bd);
      b2FixtureDef *fd = new b2FixtureDef;
      fd->density = 1.f;
      ...
      for (int i = 0; i < 2; ++i)
    	{
    	  ...
    	  ballbd.position.Set(-34.2f + i*9.0, 20.6f);
    ...
    //The pulley system
    {
      ...
      b2Vec2 worldAnchorOnBody1(-37, 10); // Anchor point on body 1 in world axis
      b2Vec2 worldAnchorOnBody2(-27, 10); // Anchor point on body 2 in world axis
      b2Vec2 worldAnchorGround1(-37, 15); // Anchor point for ground 1 in world axis
      b2Vec2 worldAnchorGround2(-27, 15); // Anchor point for ground 2 in world axis
